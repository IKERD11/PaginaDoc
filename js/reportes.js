...
function generarReporteGeneral(fechaInicio, fechaFin) {
    const estadisticas = obtenerEstadisticas();
    const usuarios = obtenerUsuarios({ rol: 'alumno' });
    ...
    const reporte = {
        ...
        detalles: {
            alumnos: usuarios.map(u => ({
                ...
                nombre: u.nombre,
                email: u.email,
                estado: obtenerEstadoDocumentacion(u.numeroControl)
            }))
        }
    };
...
function generarReporteDocumentos(fechaInicio, fechaFin) {
    ...
    if (fechaInicio || fechaFin) {
        documentos = documentos.filter(d => {
            const fechaDoc = new Date(d.fechaCarga);
...
function generarReporteAlumnos() {
    ...
    switch (reporte.tipo) {
        case 'documentos':
            ...
            reporte.documentos.forEach(d => {
                const usuario = obtenerUsuarioPorNumeroControl(d.numeroControl);
                csv += `${d.numeroControl},${usuario ? usuario.nombre : 'Desconocido'},${d.tipoDocumento},${d.estado},${formatearFechaHora(d.fechaCarga)},"${d.observaciones}"\n`;
            });
            break;
